...
audio_window = Tk()
audio_window.geometry ( '350x200' )
audio_window.title ('Audio Cryptography')
lbl1 = Label(audio_window, text = "Select bit(8/16): " ).place( x=20, y=40 )
...
def encrypt():
	...
	try:
		if(messagebox.askyesno('Question', 'Is it a 16bit audio file? ') == True): #16bit audio file
			start = time.time()
...
def decrypt():
	...
	try:
		...
		if(messagebox.askyesno('Question', 'Is it a 16bit audio file?') == True): #16bit
...
btn_decrypt = Button( audio_window, text = "Audio Decryption" , command = decrypt).place( x=20, y=100 )
btn_encrypt = Button( audio_window, text = "Audio Encryption" , command = encrypt).place( x=200, y=100 )
